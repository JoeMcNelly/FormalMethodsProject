...
pred SequenceCheck[d:Data, s,s':State] {
	(One in d.sequenceNumber and One in s.packetStatus and Zero in s'.packetStatus) and 
	(Zero  in d.sequenceNumber and Zero in s.packetStatus and One in s'.packetStatus)
}

pred ACK[s,s',s'':State, d:Data]{ 
	one d':Data | 
	(
		(ErrorCheck[d',s,s'] and SequenceCheck[d,s,s''] and d' in Ack ) implies s'.send = s.send - {d}
	)
...
run Possible for 10 State, exactly 5 Data,  5 CheckSum
...
check AlwaysSend for 10 State, 5 Data, 5 CheckSum, exactly 1 Nak
